"""A right-to-left arithmetic expression evaluator for coundetrip fixtures.

Supports ``+``, ``-``, ``*``, ``/``, parentheses, and unary minus over integer
and floating-point literals. Unlike conventional arithmetic, the binary
operators have *no precedence* and associate *right to left*, as in APL: an
expression ``a op b op c`` is evaluated as ``a op (b op c)``. Parentheses group
explicitly. The implementation is a hand-written recursive-descent parser;
...
def evaluate(expression: str) -> int | float:
    ...
    Operators carry no precedence and associate right to left. Integer-only
    ``+``, ``-`` and ``*`` keep integer results; ``/`` performs true division
    and so always yields a float. Raises ``ValueError`` for malformed input and
    propagates ``ZeroDivisionError`` for division by zero.
    """
...
class _Parser:
    ...
    def parse_expr(self) -> int | float:
        # No precedence, right-associative: a op (b op (c ...)).
